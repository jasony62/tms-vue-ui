...
var Creator = /*#__PURE__*/function () {
  ...
  (0, _createClass2.default)(Creator, [{
    ...
    value: function createLabelAndDesc(field, inputElement) {
      ...
      if (field.label) {
        var labelNode = new _nodes.LabelNode(vm, createElement, field);
        var labelNodes = [];

        if (_nodes.components.label.option.native) {
    ...
    value: function createNodeByField(fields, key) {
      ...
      var node = (0, _nodes.prepareFieldNode)(vm, createElement, field);
    ...
    value: function render() {
      ...
      if (schema.title) {
        nodesTopLevel.push(createElement(_nodes.components.title.tag, schema.title));
      }

      if (schema.description) {
        nodesTopLevel.push(createElement(_nodes.components.description.tag, schema.description));
      }

      if (vm.error) {
        var errorNodes = [];

        if (_nodes.components.error.option.native) {
          errorNodes.push(vm.error);
        }

        var errorNode = new _nodes.Node(vm, createElement, _nodes.components.error);
      ...
      var formNode2 = new _nodes.FormNode(vm, createElement);
