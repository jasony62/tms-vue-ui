...
describe('flex', () => {
  ...
  it('渲染组件-指定占据剩余空间的子元素', () => {
    const comp = Vue.component('test', {
      render(h) {
        return h('tms-flex', { props: { growItems: [1] } }, [h('div'), h('div'), h('div')])
      }
    })
    const wrapper = mount(comp)
    expect(wrapper.html()).toBe(
      '<div class="tms-flex tms-flex_row"><div class="tms-flex__item"></div><div class="tms-flex__item tms-flex__item_grow_1"></div><div class="tms-flex__item"></div></div>'
    )
    wrapper.destroy()
  })
  it('渲染组件-嵌套', () => {
