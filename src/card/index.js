  ...
  Vue.component('tms-card', {
    ...
    render() {
      ...
      function onClick() {
        ...
        else if (to && that.$parent && that.$parent.$router) this.$parent.$router.push(to)
      }
      return (
        <div class="tms-card" onClick={onClick} {...{ style: { backgroundColor: backColor } }}>
          <tms-flex {...{ props: { direction: 'column', gap: this.gap } }}>
            {slots.header ? <header>{slots.header}</header> : ''}
            <main>
              <tms-flex {...{ props: { elasticItems: [1], gap: this.gapMain } }}>
                {thumb === false ? (
                  ''
                ) : (
                ...
                <div class="tms-card__content">
                  <tms-flex {...{ props: { direction: 'column', elasticItems: [1], gap: this.gapContent } }}>
                    <div class="tms-card__title">{slots.title ? slots.title : title}</div>
                    <div class="tms-card__desc" {...{ style: { color: descColor } }}>
